...
- 首屏只调用 `index` 的一页结果；必须使用 `nextOffset` 游标逐页读取，不把全库正文放入上下文。`search` 先缩小范围，再 `read` 相关文档。
- 批量整理按批次记录范围、游标、成功、失败、跳过及原因；中断后从最后一个已确认游标继续，不重复创建。一次批次只提交有限并发的 `put`，遇到 `LOCK_BUSY` 退避重试。
- 写入前按完整 KB-ID、稳定对象编号、来源和正文摘要查重；同一对象的真实变更更新原文档，不能静默覆盖或重复累计。
- 单篇正文接近 256 KiB 时按章节或对象拆分，并在正文互相引用；不要靠截断丢失条件。详细来源放正文来源表，`sources` 只保留短引用。
- 分类使用稳定主分类；新分类先与现有分类比较，近义分类只提出归并建议，不自动重分类历史文档。
- `reindex` 是全量维护操作，适合低峰期或批次结束执行；写入成功但收到 warning 时，明确说明正文已保存、索引待修复，不能重复创建。
- `.history` 和 `.history/_deleted` 只作审计留档，不提供 Skill 恢复命令；管理员需另行制定保留周期和空间清理计划。
- 所有响应和缓存必须绑定当前 `account_id`、`agent_uid`、`instance_id`；发现离线、索引重建或权限错误时返回稳定错误，不能用 0 条代替。
- 删除仍必须先 `read` 核对 revision；格式损坏文件使用 `delete-raw` 核对路径和 SHA-256。Wiki 始终只读，不能绕过 Skill 删除。
## 更新与删除
