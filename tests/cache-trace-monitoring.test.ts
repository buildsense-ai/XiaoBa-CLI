...
test('dashboard exposes a discoverable cache trace page', () => {
  ...
  assert.match(page, /Cache Lens/);
  assert.match(page, /catsco\.dashboardApiKey/);
  assert.match(page, /采集 Cache Trace/);
  assert.match(page, /stablePrefixEstimatedTokens/);
  assert.match(page, /所选请求的数字拆解/);
  assert.match(page, /比上一条请求/);
  assert.match(page, /这条记录里变了什么/);
  assert.match(page, /稳定前缀（估算）/);
  assert.match(page, /requestOrigin==='main'/);
  assert.match(page, /requestKind==='main_inference'/);
  assert.match(page, /requestKind==='checkpoint_compaction'/);
  assert.match(page, /qualification\?\.eligible===true/);
  assert.match(page, /slice\(-12\)/);
  assert.match(page, /cacheReadTokens/);
  assert.match(page, /inputTokens/);
  assert.match(page, /Memory Branch/);
  assert.match(page, /Subagent/);
  assert.match(page, /基线已重置/);
  assert.match(page, /稳定系统提示未变化/);
  assert.match(page, /\/api\/cache-trace\/config/);
  assert.match(page, /\/api\/cache-trace\/sessions/);
  assert.match(page, /\/api\/cache-trace\/session\//);
  assert.doesNotMatch(page, /缓存命中监控/);
  assert.doesNotMatch(page, /OpenAI 显式稳定前缀/);
  assert.doesNotMatch(page, /Anthropic 分层显式断点/);
});
