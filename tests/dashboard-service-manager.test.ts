...
describe('dashboard service manager', () => {
  ...
  test('development resolves a PATH node.cmd shim to its real executable on Windows', (t) => {
    if (process.platform !== 'win32') {
      t.skip('Windows command shims are platform-specific');
      return;
    }
    ...
    const previousEnv = new Map(envKeys.map(key => [key, process.env[key]]));
    const runtimeRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'xiaoba-empty-runtime-'));
    const shimRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'xiaoba-node-shim-'));
    const realNodeRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'xiaoba-real-node-'));
    const realNode = path.join(realNodeRoot, 'node.exe');
    ...
    process.env.Path = [
      shimRoot,
      realNodeRoot,
      previousEnv.get('Path') || previousEnv.get('PATH') || '',
    ...
    try {
      ...
      fs.rmSync(realNodeRoot, { recursive: true, force: true });
    }
  });
