...
describe('AgentSession lifecycle', () => {
  ...
  test('handleMessage surfaces relay budget exhaustion without leaking raw provider errors', async () => {
    ...
    assert.match(result.text, /切换到还有额度的模型/);
  ...
  test('handleMessage surfaces account budget exhaustion when model config is unavailable', async () => {
    ...
    assert.match(result.text, /CatsCompany 中转页面/);
  });
  ...
  test('handleMessage surfaces transient provider failures without leaking raw upstream payload', async () => {
    ...
    assert.match(result.text, /本次未记录到自动重试/);
    assert.match(result.text, /对话上下文已保存/);
  ...
  test('handleMessage maps rate limits to a user-facing message without exposing status codes', async () => {
    ...
    assert.match(result.text, /当前模型请求过于频繁/);
    assert.match(result.text, /本次未记录到自动重试/);
    assert.match(result.text, /对话上下文已保存/);
  ...
  test('handleMessage mentions an automatic retry only when retry metadata confirms one occurred', async () => {
    ...
    assert.match(result.text, /当前模型服务临时异常/);
    assert.match(result.text, /系统已自动重试 1 次，仍未恢复/);
    assert.match(result.text, /对话上下文已保存/);
