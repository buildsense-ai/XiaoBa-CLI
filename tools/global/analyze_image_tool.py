...
class AnalyzeImageTool(BaseTool):
    ...
    tool_description = "读取图片文件并调用多模态视觉模型进行分析，返回纯文字结果。支持 OpenAI 和 Anthropic 视觉模型，带主备链路自动切换。"
    tool_parameters = {
        "type": "object",
        "properties": {
            "file_path": {
                "type": "string",
                "description": "图片文件路径（支持绝对路径和相对路径）"
            },
            "prompt": {
                "type": "string",
                "description": "分析提示词，描述你希望模型关注的内容"
            },
            "detail": {
                "type": "string",
                "description": "图片分析精度：auto / low / high",
                "enum": ["auto", "low", "high"],
                "default": "auto"
            },
            "max_tokens": {
