...
内置 Skill 仅在调用时提供当前运行体的 `data`、`logs/sessions` 绝对目录和运行时 session ID（缺失显示 null），不把会话 ID 写入稳定系统提示或 Skill 列表，也不从模型参数接受替代身份。用户同名自定义 Skill 不被改写。

知识仍使用现有 `sources: string[]` 与正文“来源与核验”章节，不新增业务数据库或自动会话归档。说明要求记录实际来源会话/记忆 ref、文件原路径及持久副本位置、工作表行号或PDF页码，并区分生效时间与保存时间。更新保留旧来源，追加本次变更来源；当前轮日志尚未落盘时不猜造 turn/ref。具体记录与回溯流程以 `skills/xiaoba-knowledge/SKILL.md` 为准。

为减少来源信息重复，来源项可使用本文内S1/S2编号，正文按“编号+行/页/章节”引用；共用来源的表格统一声明，混合来源逐项标明。编号含义在更新时保留，不是跨文档ID。原件/用户原话用于核对事实，关联日志用于还原过程，助手总结不能自证。精确引用优先直达原件，必要时再查日志；版本标识或哈希区分来源版本，不承诺恢复被删除的原件。

来源坐标须保留原工作表行号/单元格或PDF页码，不用过滤空行、表头后的数组索引代替；保存前回读原件核对定位，未知坐标可用已核对的业务编号/章节定位并说明缺口。金额之外，“已结清/已全额开票/逾期”等派生状态也须从明细按条件计算并与文字概述对照；合计正确不代表逐项状态正确。上述要求来自本地真实验收发现的行号偏移和状态概述矛盾，仍需模型实测确认遵循情况。

`data/sessions` 是可压缩或重置的恢复快照；`logs/sessions` 的 turn 记录与附件才是回溯入口，仍需检查存在性和完整性。此次改动不延长日志或附件保留期，不保证来源永久可用，也不自动为旧知识补齐来源。已定位的来源应直接读取；没有精确引用时再按会话/时间/关键词检索，避免反复整库扫描。来源追踪不能代替数值校验；Skill 要求程序计算及保存后核对，但 helper 没有自动验证业务金额。

### 在转成知识时组织和核验

Skill按需读取随包分发的 `skills/xiaoba-knowledge/references/business-material-types.md`，提供对象档案、销售、合同、订单、财务、产品、项目、流程、制度、人事、客服排障、技术环境、物流资产、沟通决策、研究数据、内容素材等类型及保留重点。所属对象、资料类型、时间/版本分别表达；category仍是单个粗分类，其余线索放入标题、summary和正文。优先复用含义一致的已有分类，未知类型允许模型按用途自定或暂用中性描述，不能强套模板、编造字段或按分类复制全文。这是分类指导，不新增业务schema、自动分类器或按公司隔离权限；旧知识不会自动迁移。

已有示例和既有分类不准确时也允许按资料用途自行纠正，用户明确指定的分类优先；自定义或修正后在正文标清资料类型和分类含义。调整旧知识分类在change记录原因，保留原KB-ID、有效事实和来源，不要求为统一名称重写无关知识。

Skill 将首次转换作为整理入口：明确资料覆盖范围，复用本轮解析结果，以来源中的稳定对象编号和主题组织明细；标题、summary及正文检索词提供准确名称、已知简称、编号和相关查询词。小记录可组合保存，summary承担导航作用，减少重复改写明细和合计。来源事实、派生结果及其日期/计算口径分开记录；精确金额使用整数最小货币单位或精确十进制计算，保存前核对记录覆盖、唯一编号、关键字段与合计，保存后仍要读回确认。

增量资料先辨别重复、变更及其生效日期，再提交合并后的完整put请求；大资料按章节或对象组拆分至单文件256 KiB限制内，保留跨篇引用和处理进度。尚未解析或核验的范围不能标为完成。查询全量汇总须确认相关明细齐全，不能仅以第一页搜索命中作为计算输入；正常问答可复用已验证且适用的知识，减少反复读整批原件和制作无关报告。

这些是Agent使用流程要求，不是自动解析、分块、唯一性约束、数值验证或性能优化引擎。本次未添加全文/语义索引，也未改变现有全库扫描检索；提示更完整不代表已经证明更快或更准，需发布后以真实会话对照验收。

程度词不设跨任务固定阈值：简洁表达须保留事实条件及来源，拆分使用实际字节限制与章节边界，完整性按已声明范围核对。只有歧义会改变范围、时间口径或保留内容且上下文无法确定时才澄清，避免普通措辞引发反复追问。该说明减少程度词对关键决策的影响，不保证消除模型理解差异。
...
用户可直接编辑 Markdown 正文，保留 frontmatter 和文档 ID；直接编辑不会自动生成历史版本，可运行 reindex 更新索引。Agent 更新和删除都必须使用 helper：删除先 read 获取完整 KB-ID 与 revision，再调用 Skill 的 delete，归档版本后重建索引；禁止用 execute_shell、write_file 或 edit_file 绕过版本检查直接删除正文、历史或索引。格式损坏或缺少 KB-ID 的文档使用 Skill 的 delete-raw（documents/ 相对路径 + SHA-256），原始内容归档到 .history/_deleted 后再重建索引；哈希变化或路径越界时拒绝。Wiki 只展示和发起请求，不提供直接删除入口。删除归档只用于审计，不在 Skill 中提供恢复操作，以免长期占用并复制整套知识库；确需恢复时由管理员依据归档执行独立变更。操作不支持知识根目录及其内部的符号链接/目录联接和硬链接文件。
...
数百至数千篇时，Wiki 和 Agent 都必须使用分页游标，先读元数据再按需读正文；批量导入记录游标、成功/失败/跳过原因并支持断点续跑。写入前按稳定对象编号与来源查重，正文接近 256 KiB 时按章节拆分。全量 reindex 放在批次结束或低峰期执行，warning 要区分正文已保存与索引待修复。缓存键绑定 account_id、agent_uid、instance_id，不能把离线、权限错误或索引重建显示为零条。历史归档只用于审计，保留周期和空间清理由管理员另行处理。
