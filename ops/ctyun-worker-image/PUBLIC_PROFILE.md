...
第一轮使用干净的 Ubuntu 公共镜像，不能选择带历史工作身份的业务镜像。`BASE_IMAGE_HARDENED` 首轮为 false；如有需要钉住的已验收或回滚镜像，加入该方案自己的保护名单。保护名单为空时清理照常执行（仅保留最新 N 个），并输出 warning 提示未声明保护——生产供给与重置始终选择最新镜像，旧镜像无需长期保护。
