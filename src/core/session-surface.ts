...
const CATSCO_REPLY_STYLE_INSTRUCTION = [
  ...
  '- HTML 产物要自包含，首屏内容应直接可读；不要依赖外部 CDN、fetch、localStorage、cookie 或网络请求才能显示主体内容。',
  '- 如果 HTML 需要交互或时间显示，使用内联 JS 做渐进增强，并在 DOMContentLoaded 后初始化；不要把主体内容长期留在“加载中”等待脚本填充。',
