...
export class FeishuBot {
  ...
  private async onMessage(data: any): Promise<void> {
    ...
    if (session.isBusy()) {
      ...
      queue.push({ userText, chatId: msg.chatId, senderId: msg.senderId, runtimeFeedback, source: 'user' });
  ...
  private async drainMessageQueue(sessionKey: string): Promise<void> {
    ...
    const mergedText = messages.length === 1
      ? messages[0].userText
      : messages.map((m, i) => `[${this.formatQueuedMessageLabel(m, i)}] ${m.userText}`).join('\n');
