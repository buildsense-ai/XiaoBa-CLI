...
export function getWeixinChannelStatus(options: {
  ...
  const runtime = resolveCatsCoRuntimeConfig({ runtimeRoot, env: options.env });
  ...
  const tokenConfigured = Boolean(String(options.env?.WEIXIN_TOKEN || '').trim());
...
export function bindWeixinChannelToCurrentAgent(options: {
  ...
  if (agent.bodyId) process.env.WEIXIN_BOUND_BODY_ID = agent.bodyId;
  if (agent.ownerUid) process.env.WEIXIN_BOUND_BY_USER_UID = agent.ownerUid;
