...
function buildModelStartupSnapshot(
  ...
  const envSource = firstNonEmpty(fileEnv.CATSCO_MODEL_SOURCE, env.CATSCO_MODEL_SOURCE);
  const source = envSource === 'relay' || isCatsRelayApiBase(effective.apiBase) ? 'relay' : 'custom';
