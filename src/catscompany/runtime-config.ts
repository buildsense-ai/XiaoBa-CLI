...
export function resolveCatsCoRuntimeConfig(
  ...
  const apiKey = firstNonEmpty(auth.apiKey, config.catscompany?.apiKey);
  ...
  const confirmedLocalBotBinding = hasConfirmedLocalBotBinding(localConfig);
  ...
  const unconfirmedBotBinding = Boolean(rawBotUid && apiKey && serverUrl && !bodyConfigured);
...
function hasConfirmedLocalBotBinding(localConfig: CatsCoLocalConfig): boolean {
