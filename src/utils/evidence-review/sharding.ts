...
/**
 * Reject mutated content under an existing hash.
 * Throws when content no longer matches the declared contentHash.
 */
export function assertShardContentImmutable(shard: EvidenceShard): void {
  if (!verifyShardContent(shard)) {
    throw new Error(
      `Evidence Shard ${shard.shardId} content does not match contentHash ${shard.contentHash}`,
    );
  }
}

/** Verify every manifest member is present, hashed, and immutable. */
export function assertManifestShardsConsistent(
  manifest: EvidenceBundleManifest,
  shards: readonly EvidenceShard[],
): void {
  if (manifest.shardIds.length !== shards.length) {
    throw new Error('Evidence Bundle Manifest shard count does not match shards');
  }
  if (manifest.contentHashes.length !== shards.length) {
    throw new Error('Evidence Bundle Manifest contentHashes length does not match shards');
  }
  const byId = new Map(shards.map(s => [s.shardId, s]));
  for (const [index, shardId] of manifest.shardIds.entries()) {
    const shard = byId.get(shardId);
    if (!shard) throw new Error(`Evidence Bundle Manifest missing shard ${shardId}`);
    if (shard.contentHash !== manifest.contentHashes[index]) {
      throw new Error(`Evidence Bundle Manifest content hash mismatch for ${shardId}`);
    }
    assertShardContentImmutable(shard);
  }
  ...
  if (recomputed !== manifest.manifestHash) {
    throw new Error('Evidence Bundle Manifest hash does not match membership');
  }
}
