  ...
  recordContextModules(messages: Message[], tools: ToolDefinition[], recallMeta?: { factsCount?: number } | null): void {
    ...
    for (const msg of messages) {
      ...
      if (msg.role === 'system') {
        ...
        else if (c.includes('[long_term_memory]') || c.includes('[transient_memory_context]')) modules.recall = { tokens: t, content: c, facts_count: recallMeta?.factsCount ?? 0 };
        ...
        else if (c.includes('[previous_session_summary]')) modules.session_summary = { tokens: t, content: c };
